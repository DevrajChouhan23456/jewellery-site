...
export type JsonBodyParseFailure<T> = Exclude<
  JsonBodyParseResult<T>,
  { success: true }
>;
...
type JsonBodyErrorResponseOptions = {
  includeFieldErrors?: boolean;
  validationMessage?: string;
};

export function jsonBodyErrorResponse<T>(
  result: JsonBodyParseFailure<T>,
  options: JsonBodyErrorResponseOptions = {},
) {
  const { includeFieldErrors = false, validationMessage } = options;
  ...
  return NextResponse.json(
    {
      error: validationMessage ?? getZodErrorMessage(result.error),
      ...(includeFieldErrors
